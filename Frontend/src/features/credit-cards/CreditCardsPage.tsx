...
import { useCreditCards, useAddCreditCard } from './hooks';
            ...
            <div className="p-6 space-y-10 animate-enter">
                {showForm && (
                    <div className="glass-card p-6 rounded-[2.5rem] border-white/10 space-y-6">
                        <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-[4px] ml-1">New Card Matrix</h3>
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="flex gap-3 pt-4">
                                <button type="submit" className="flex-1 py-4.5 rounded-[1.5rem] bg-white text-black font-black uppercase text-xs tracking-widest active:scale-95 transition-all">
                                    {addCardMutation.isPending ? 'Syncing...' : 'Commit'}
                                </button>
                                <button
                                    type="button"
                ...
                <div className="grid gap-6">
                    {cards?.map((card) => (
                        <CycleInfoCard key={card.id} cardId={card.id} initialData={card} />
                    ))}
                </div>
